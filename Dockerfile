...
FROM install-browser AS Auto_Research_Engine-install
...
FROM Auto_Research_Engine-install AS Auto_Research_Engine
...
RUN useradd -ms /bin/bash Auto_Research_Engine && \
    chown -R Auto_Research_Engine:Auto_Research_Engine /usr/src/app && \
    # Add these lines to create and set permissions for outputs directory
    mkdir -p /usr/src/app/outputs && \
    chown -R Auto_Research_Engine:Auto_Research_Engine /usr/src/app/outputs && \
    chmod 777 /usr/src/app/outputs
USER Auto_Research_Engine
WORKDIR /usr/src/app

# Copy the rest of the application files with proper ownership
COPY --chown=Auto_Research_Engine:Auto_Research_Engine ./ ./
