const GPTResearcher = (() => {
  ...
  const loadResearchEntry = (index) => {
    ...
    // Keep report type pinned to multi_agents
    const reportTypeSelect = document.querySelector('select[name="report_type"]');
    if (reportTypeSelect) {
        const savedReportType = entry.reportType;
        reportTypeSelect.value = savedReportType === 'multi_agents' ? savedReportType : 'multi_agents';
    }
  ...
  const listenToSockEvents = () => {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.type === 'logs') {
        ...
        // Append HTML of current chunk to the container for multi_agents mode.
    ...
    socket.onopen = (event) => {
      ...
      const reportTypeSelect = document.querySelector('select[name="report_type"]');
      if (reportTypeSelect) {
        reportTypeSelect.value = 'multi_agents';
      }
      const report_type = 'multi_agents'
