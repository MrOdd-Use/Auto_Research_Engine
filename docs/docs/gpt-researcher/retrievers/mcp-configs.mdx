...
The Model Context Protocol (MCP) enables Auto_Research_Engine to connect with diverse data sources and tools through a standardized interface. Auto_Research_Engine features an intelligent two-stage MCP approach that automatically selects the best tools and generates contextual research, powered by LangChain's [MCP adapters](https://github.com/langchain-ai/langchain-mcp-adapters) for seamless integration.

## How MCP Works in Auto_Research_Engine

Auto_Research_Engine uses a **two stage intelligent approach** for MCP integration:
...
3. **Initialization**: Auto_Research_Engine loads your `mcp_configs`
...
MCP support is included with Auto_Research_Engine installation:
...
**Important:** To use MCP with Auto_Research_Engine, you must set the `RETRIEVER` environment variable:
...
# Auto_Research_Engine automatically orchestrates all servers
context = await researcher.conduct_research()
report = await researcher.write_report()
...
This example demonstrates how Auto_Research_Engine intelligently:
...
Auto_Research_Engine automatically detects connection types:
...
*For more examples and advanced use cases, check out the [Auto_Research_Engine examples repository](https://github.com/MrOdd-Use/Auto_Research_Engine/tree/master/examples).* :-) 