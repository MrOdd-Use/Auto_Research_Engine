...
The Auto_Research_Engine package allows you to integrate with existing langchain vector stores that have been populated.
...
Auto_Research_Engine will work with any langchain vector store that implements the `asimilarity_search` method.
...
In some cases in which you want to store the scraped data and documents into your own vector store for future usages, Auto_Research_Engine also allows you to do so seamlessly just by inputting your vector store (make sure to set `report_source` value to something other than `langchain_vectorstore`)
