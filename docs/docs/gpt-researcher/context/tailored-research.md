...
The Auto_Research_Engine package allows you to tailor the research to your needs such as researching on specific sources (URLs) or local documents, and even specify the agent prompt instruction upon which the research is conducted.

### Research on Specific Sources 📚

You can specify the sources you want the Auto_Research_Engine to research on by providing a list of URLs. The Auto_Research_Engine will then conduct research on the provided sources via `source_urls`. 

If you want Auto_Research_Engine to perform additional research outside of the URLs you provided, i.e., conduct research on various other websites that it finds suitable for the query/sub-query, you can set the parameter `complement_source_urls` as `True`. Default value of `False` will only scour the websites you provide via `source_urls`.
...
You can instruct the Auto_Research_Engine to research on local documents by providing the path to those documents. Currently supported file formats are: PDF, plain text, CSV, Excel, Markdown, PowerPoint, and Word documents.
...
Auto_Research_Engine will then conduct research on the provided documents.
...
### Hybrid Research 🔄
You can combine the above methods to conduct hybrid research. For example, you can instruct the Auto_Research_Engine to research on both web sources and local documents.
...
To learn more about retrievers check out the [Retrievers](https://docs.gptr.dev/docs/Auto_Research_Engine/search-engines/retrievers) documentation.


### Research on LangChain Documents 🦜️🔗
You can instruct the Auto_Research_Engine to research on a list of langchain document instances.
