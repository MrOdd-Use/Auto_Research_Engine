...
This function sends a Webhook Message to the GPTR Python backend running on localhost:8000, but this example can also be modified to query a [GPTR Server hosted on Linux](https://docs.gptr.dev/docs/Auto_Research_Engine/getting-started/linux-deployment).
...
async function sendWebhookMessage(message) {
  return new Promise((resolve, reject) => {
    ...
    const data = {
      ...
      tone: 'Objective',
      headers: {},
      repo_name: 'elishakay/Auto_Research_Engine'
    };
...
async function main() {
  const message = 'How do I get started with Auto_Research_Engine Websockets?';
